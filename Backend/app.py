...
async def send_email(candidate_email: str):
    ...
    try:
        ...
        SMTP_PORT = 587
        EMAIL_SENDER = "sathyaprakashmsd@gmail.com"
...
async def get_all_resumes():
    """
    Returns all resumes stored in MongoDB.
    (Each document is expected to have fields: email, filename, match_score, etc.)
    """
    try:
        all_resumes = list(collection.find({}, {"_id": 0}))
        return JSONResponse(content={"resumes": all_resumes})
    except Exception as e:
        logger.exception("Error retrieving resumes")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/admin/schedule")
async def schedule_interview(data: dict):
    """
    Schedules an interview by sending an email to the candidate.
    Expects a JSON body with:
      - candidate_email: string
      - interview_datetime: string (in a displayable format, e.g. ISO format)
    """
    candidate_email = data.get("candidate_email")
    interview_datetime = data.get("interview_datetime")
    
    if not candidate_email or not interview_datetime:
        raise HTTPException(status_code=400, detail="Candidate email and interview datetime are required")
    ...
    try:
        ...
        msg["From"] = EMAIL_SENDER
        msg["To"] = candidate_email
        msg["Subject"] = "Interview Scheduled - Faculty Position at Chennai Institute of Technology"

        body = f"""
Dear Candidate,

We are pleased to inform you that your interview for the faculty position at Chennai Institute of Technology has been scheduled.
...
"""
        ...
        return JSONResponse(content={"message": "Interview scheduled and email sent successfully!"})
    except Exception as e:
